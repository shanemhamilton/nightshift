...
- **Compose** new automation suites from an adaptive 7-pattern library (coverage ratchet, product-value loop, repo-hygiene integrator, leftover resolver, collaboration meta-learner, code-simplification, code-security), wired into a coordinated fleet with exactly one merge authority. Templates are capability-driven: they detect what a project supports and degrade gracefully, never guessing commands.
