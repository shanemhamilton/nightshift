...
- **Compose** new automation suites from an adaptive 9-pattern library (coverage ratchet, product-value loop, repo-hygiene integrator, leftover resolver, collaboration meta-learner, code-simplification, code-security, dev-environment self-reflection, documentation-sync ratchet), wired into a coordinated fleet with exactly one merge authority and de-clustered schedules. Templates are capability-driven: they detect what a project supports and degrade gracefully, never guessing commands. A fleet-global pattern (**P9**) collapses every project's pending human decisions into one daily `~/.codex/DAILY-APPROVALS.md` inbox.
