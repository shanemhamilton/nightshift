...
def check_managed_block_v4_shape(root: Path) -> None:
    job_dir = root / "shape-check"
    job_dir.mkdir()
    block = OPT.managed_block(str(job_dir))
    check("managed_block v4 contains 'Protocol version: 4'",
          "Protocol version: 4" in block)
    missing = [s for s in OPT.REQUIRED_SECTIONS if s not in block]
    check("managed_block v4 contains all REQUIRED_SECTIONS", not missing, missing)
...
def check_newer_than_helper(root: Path) -> None:
    ...
    block = OPT.managed_block(str(job_dir)).replace(
        "Protocol version: 4", "Protocol version: 99"
    )
    prompt = block + "\n\nTask body.\n"
...
def main() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        check_managed_block_v4_shape(root)
